...
extraer_datos_medias <- function(ruta, target_var) {
  ...
  # ⚠️ AJUSTA ESTAS COLUMNAS SEGÚN TU ARCHIVO
  mean_control <- clean(df[fila_idx, 3])
  mean_treated <- clean(df[fila_idx, 5])
...
for (v in variables_tesis) {
  ...
  dataset_temp <- map_df(archivos, ~extraer_datos_medias(.x, v)) %>% 
    arrange(periodo)
  
  if(nrow(dataset_temp) > 0) {
    
    dataset_long <- dataset_temp %>%
      pivot_longer(cols = c(control, treated),
                   names_to = "grupo",
                   values_to = "media")
    
    p <- ggplot(dataset_long, aes(x = periodo, y = media, color = grupo, group = grupo)) +
      geom_line(size = 1) +
      geom_point(size = 2) +
      labs(title = paste("Means over time:", v),
           x = "Quarter", y = "Mean",
           color = "Group") +
      theme_minimal() +
      theme(axis.text.x = element_text(angle = 45, hjust = 1))
    
    nombre_archivo <- paste0("means_", v, ".png")
    ruta_final <- file.path("~/Desktop/1 economia/thesis_economics/data/controles_results/graficas", nombre_archivo)
